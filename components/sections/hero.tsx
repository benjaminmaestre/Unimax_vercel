...
export function HeroSection() {
  ...
  return (
    <section id="hero" className="relative h-screen min-h-[680px] overflow-hidden grain bg-black text-white">
      ...
      <div className="absolute inset-0">
        ...
        />
        {/* Cinematic Overlays (Locked as dark for text contrast and industrial atmosphere in all modes) */}
        <div 
      ...
      <div className="absolute right-0 top-1/3 -translate-y-1/2 pointer-events-none select-none overflow-hidden hidden lg:block max-w-full z-0">
        <span className="font-display text-[180px] font-black leading-none text-white/12 text-outline tracking-wider block translate-x-16 uppercase">
          {t('hero.bg_decor')}
        </span>
      </div>
      ...
      <div className="relative z-10 section-container h-full flex flex-col lg:flex-row items-center justify-between gap-10">
        ...
        <div className="max-w-[720px] flex flex-col justify-center">
          ...
          >
            ...
            <motion.span 
              ...
              className="text-[12px] font-extrabold tracking-[0.25em] uppercase text-white/90 mb-5 flex items-center gap-3"
            >
            ...
            <h1 className="font-display select-none text-white leading-tight">
              <span className="block text-[42px] sm:text-[56px] lg:text-[68px] font-extrabold tracking-tight drop-shadow-2xl">
                {t('hero.title1')}
              </span>
              <span className="block text-[30px] sm:text-[40px] lg:text-[48px] font-bold text-white/90 tracking-tight mt-2 drop-shadow-xl font-sans">
                {t('hero.title2')}
              </span>
            </h1>
            ...
            <motion.p 
              ...
              className="mt-6 text-base sm:text-lg lg:text-xl leading-[1.65] text-white/80 max-w-[620px] font-light"
            >
              {t('hero.desc')}
            </motion.p>
            ...
            <div className="mt-10 flex flex-col sm:flex-row gap-4">
              ...
              <Link
                href="#soluciones"
                className="inline-flex items-center justify-center h-[56px] px-9 text-[12px] font-bold tracking-[0.15em] uppercase bg-transparent text-white border-2 border-white/40 hover:border-white/80 rounded-md hover:bg-white/5 transition-all duration-200 active:scale-95"
              >
        ...
        <motion.div
          ...
          className="hidden md:flex flex-col items-start gap-4 p-6 bg-white/3 border border-white/10 backdrop-blur-md rounded-xl max-w-[320px] shadow-2xl relative overflow-hidden group"
        >
          {/* Decorative glow */}
          ...
          <h3 className="font-display text-xl text-white mt-1 leading-tight tracking-wide">
            {language === 'es' ? '4 PLANTAS DOSIFICADORAS DESPACHANDO 24/7' : '4 BATCHING PLANTS DISPATCHING 24/7'}
          </h3>
          
          <p className="text-[11px] text-white/70 leading-relaxed font-light mt-1">
            {language === 'es' 
          ...
          <Link
            href="#plantas" 
            className="inline-flex items-center gap-2 text-[10px] font-bold tracking-[0.15em] uppercase text-primary hover:text-white transition-colors duration-200 mt-2 cursor-pointer"
          >
      ...
      >
        <span className="text-[9px] font-bold tracking-[0.3em] uppercase text-white/50">{t('hero.scroll')}</span>
      ...
      <motion.div
        ...
        className="absolute bottom-0 left-0 right-0 bg-black/75 border-t border-white/15 backdrop-blur-md hidden lg:block z-20"
      >
            ...
            {stats.map((stat) => (
              <div key={stat.label} className="py-8 px-6 flex flex-col justify-center">
                <div className="flex items-baseline gap-1">
                  <span className="font-display text-4xl text-white tracking-tight font-extrabold">
                    {stat.value}
                  </span>
                ...
                <p className="text-[9px] font-bold tracking-[0.2em] uppercase text-white/60 mt-1">
                  {stat.label}
                </p>
              </div>
            ))}
